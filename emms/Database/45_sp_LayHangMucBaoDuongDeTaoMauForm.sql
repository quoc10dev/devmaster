...
GO
ALTER PROCEDURE sp_LayHangMucBaoDuongDeTaoMauForm
(
	@maintenanceList varchar(max)
)
AS
BEGIN
	SET NOCOUNT ON
	...
	-- chuyển danh sách các hạng mục bảo dưỡng đã chọn thành bảng
	SELECT * into #tmpHangMucDaChon FROM dbo.Split(@maintenanceList, ',')

	-- Lấy danh sách nhóm các hạng mục
	Select b.ID as IDHangMuc, b.Ten as TenHangMuc, b.SoThuTuHienThi as SoThuTuHienThi_HangMuc,
	...
	Begin
		...
		Begin
			...
			Set @STT = 1
			Select IDHangMuc, TenHangMuc, SoThuTuHienThi_HangMuc into #tmp3 From #tmpHangMucNhomHangmuc where ID = @IDNhomHangMucCon Order by SoThuTuHienThi_HangMuc
			While exists(Select 1 From #tmp3)
			Begin
				Select Top 1 @IdHangMuc = IDHangMuc, @Ten = TenHangMuc From #tmp3 order by SoThuTuHienThi_HangMuc

				Insert into #tmpHangMucBaoDuong(IDNhomHangMuc, IDNhomHangMucCha, IDHangMuc, STT, Ten, IsBold)
				values(@IDNhomHangMucCon, @IdNhomHangMucCha, @IdHangMuc, @STT, @Ten, 0)
