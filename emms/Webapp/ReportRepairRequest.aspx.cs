...
public partial class ReportRepairRequest : BaseAdminGridPage
{
    #region begin
    ...
    public override string FunctionPageCode
    {
        get
        {
            return FunctionCode.Report_Repair_Request;
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                txtFilterNgaySuaChuaTuNgay.Attributes.Add("onKeyPress", "DoClick('" + btnFilter.ClientID + "',event)");
                txtFilterNgaySuaChuaDenNgay.Attributes.Add("onKeyPress", "DoClick('" + btnFilter.ClientID + "',event)");
    ...
    private void SetDefaultDateValue()
    {
        ...
        txtFilterNgaySuaChuaTuNgay.Text = DateTimeHelper.FirstDayOfCurrentYear().ToString("dd/MM/yyyy");
        txtFilterNgaySuaChuaDenNgay.Text = DateTimeHelper.LastDayOfCurrentYear().ToString("dd/MM/yyyy");
    }
    ...
    public override void BindData()
    {
        ...
        //Ngày sửa chữa
        DateTime ngaySuaChuaTuNgay = DateTime.MinValue;
        DateTime ngaySuaChuaDenNgay = DateTime.MinValue;

        if (string.IsNullOrEmpty(txtFilterNgaySuaChuaTuNgay.Text.Trim()) || txtFilterNgaySuaChuaTuNgay.Text.Trim().Equals("__/__/____"))
        {
            ShowMessage(this, "Please enter <b>Ngày bảo dưỡng từ ngày</b> is a datetime", MessageType.Message);
            return;
        }
        else if (!DateTimeHelper.ConvertStringToDateTime(txtFilterNgaySuaChuaTuNgay.Text.Trim(), out ngaySuaChuaTuNgay))
        {
            ShowMessage(this, "Error convert <b>Ngày bảo dưỡng từ ngày</b> to datetime", MessageType.Message);
            return;
        }

        if (string.IsNullOrEmpty(txtFilterNgaySuaChuaDenNgay.Text.Trim()) || txtFilterNgaySuaChuaDenNgay.Text.Trim().Equals("__/__/____"))
        {
            ShowMessage(this, "Please enter <b>Ngày bảo dưỡng đến ngày</b> is a datetime", MessageType.Message);
            return;
        }
        else if (!DateTimeHelper.ConvertStringToDateTime(txtFilterNgaySuaChuaDenNgay.Text.Trim(), out ngaySuaChuaDenNgay))
        {
            ShowMessage(this, "Error convert <b>Ngày bảo dưỡng đến ngày</b> to datetime", MessageType.Message);
            return;
        }

        if (DateTime.Compare(ngaySuaChuaTuNgay, ngaySuaChuaDenNgay) > 0)
        {
        ...
        DataTable dt = RepairRequestManager.ReportRepairRequest(ngayNhapTuNgay, ngayNhapDenNgay, ngaySuaChuaTuNgay, ngaySuaChuaDenNgay,
    ...
    protected void gridView_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (row != null)
                {
                    HyperLink hpMaPhieu = e.Row.FindControl("hpMaPhieu") as HyperLink;
                    if (hpMaPhieu != null)
                    {
                        hpMaPhieu.Text = row["MaPhieu"].ToString();
                        hpMaPhieu.NavigateUrl = string.Format("../RepairRequestDetail.aspx?ID={0}", Convert.ToInt32(row["IDPhieuSuaChua"]));
                    }

                    HyperLink hpBienSo = e.Row.FindControl("hpBienSo") as HyperLink;
                    if (hpBienSo != null)
                    {
