...
public partial class MaintenanceChildType : BaseAdminGridPage
{
    #region begin
    ...
    {
        btnAddNew.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.ChildMaintenanceType_Add); //Phân quyền thêm
    }
    ...
    public override void BindData()
    {
        ...
        if (!string.IsNullOrEmpty(dlMaintenanceType.SelectedValue))
            idKieuBaoDuong = int.Parse(dlMaintenanceType.SelectedValue);

        DataTable dt = MaintenanceTypeManager.SearchMaintenanceChildType(idKieuBaoDuong, txtFilterName.Text.Trim(), 
    ...
    protected void gridView_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (row != null)
                {
                    Label lblSoLuongBaoDuongDinhKy = e.Row.FindControl("lblSoLuongBaoDuongDinhKy") as Label;
                    if (lblSoLuongBaoDuongDinhKy != null)
                        lblSoLuongBaoDuongDinhKy.Text = NumberHelper.ToStringNumber(row["SoLuongBaoDuongDinhKy"]);
                    ...
                    if (lnkEdit != null)
                    {
                        ...
                        lnkEdit.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.ChildMaintenanceType_Edit) ||
                            FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.ChildMaintenanceType_View); //phân quyền sửa
                    }
                    ...
                    if (lnkDelete != null)
                    {
                        ...
                        lnkDelete.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.ChildMaintenanceType_Delete); //phân quyền xóa
                    }
                    ...
                    if (hfName != null && row["TenLoaiBaoDuong"] != DBNull.Value)
                        hfName.Value = row["TenLoaiBaoDuong"].ToString();
                }
            }
        }
        catch (Exception exc)
    ...
    protected void gridView_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("editRecord"))
            {
                ...
                Response.Redirect(string.Format("MaintenanceChildTypeDetail.aspx?ID={0}", id));
            }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            MaintenanceTypeManager.DeleteMaintenanceChildType(int.Parse(hfIdToDelete.Value));
            HideModal();
            BindData();
        }
        catch (Exception exc)
