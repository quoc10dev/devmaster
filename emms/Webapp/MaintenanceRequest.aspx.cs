...
public partial class MaintenanceRequest : BaseAdminGridPage
{
    #region begin
    ...
    public override void BindData()
    {
        ...
        DataTable dt = MaintenanceRequestManager.SearchMaintenanceRequest(ngayNhapTuNgay, ngayNhapDenNgay, ngayBaoDuongTuNgay, ngayBaoDuongDenNgay,
                                idCongTy, idKieuBaoDuong, idNhomTrangThietBi, idLoaiTrangThietBi,
                                txtFilterMaThe.Text.Trim(), txtFilterBienSo.Text.Trim(), dlFilterTrangThai.SelectedValue, 
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (row != null)
                {
                    ...
                    if (lnkEdit != null)
                    {
                        ...
                        lnkEdit.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.Maintenance_Request_Edit) ||
                                FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.Maintenance_Request_View); //phân quyền sửa
                    }
                    ...
                    if (lnkDelete != null)
                    {
                        ...
                        lnkDelete.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.Maintenance_Request_Delete); //phân quyền xóa
                    }
                    ...
                    if (hfMaThe != null && row["MaThe"] != DBNull.Value)
                        hfMaThe.Value = row["MaThe"].ToString();
                }
            }
        }
        catch (Exception exc)
    ...
    protected void gridView_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("editRecord"))
            {
                ...
                Response.Redirect(string.Format("MaintenanceRequestDetail.aspx?ID={0}", id));
            }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            int idTheBaoDuong = int.Parse(hfIdToDelete.Value);
            MaintenanceRequestManager.DeleteMaintenanceRequest(idTheBaoDuong);
            
            //Cập nhật thông tin bảo dưỡng (Ngày bảo dưỡng, Số km/ Số giờ) vào bảng tblTrangThietBi
            MaintenanceRequestManager.CapNhatThongTinBaoDuong(idTheBaoDuong);
