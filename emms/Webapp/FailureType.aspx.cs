...
public partial class FailureType : BaseAdminGridPage
{
    #region begin
    ...
    {
        btnAddNew.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.Failure_Type_Add); //Phân quyền thêm
    }

    private void SetMaxLength()
    {
        txtFilterFailureName.MaxLength = 200;
    }
    ...
    {
        int totalRecord = 0;
        DataTable dt = FailureManager.SearchFailure(txtFilterFailureName.Text.Trim(), PageIndex, PageSize, SortExpression, GetSortOrder, out totalRecord);
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (row != null)
                {
                    ...
                    if (lnkEdit != null)
                    {
                        ...
                        lnkEdit.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.Failure_Type_Edit); //phân quyền sửa
                    }
                    ...
                    if (lnkDelete != null)
                    {
                        ...
                        lnkDelete.Visible = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.Failure_Type_Delete); //phân quyền xóa
                    }
    ...
    protected void gridView_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("editRecord"))
            {
                ...
                Response.Redirect(string.Format("FailureDetail.aspx?ID={0}", id));
            }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            FailureManager.DeleteFailure(int.Parse(hfIdToDelete.Value));
            HideModal();
            BindData();
        }
        catch (Exception exc)
    ...
    protected void btnClear_Click(object sender, EventArgs e)
    {
        try
        {
            txtFilterFailureName.Text = string.Empty;
        }
        catch (Exception exc)
        {
            ShowMessage(this, exc.ToString(), MessageType.Error);
