...
public partial class TransferData : BaseAdminGridPage
{
    ...
    public override string FunctionPageCode
    {
        get
        {
            return FunctionCode.Transfer_Data_From_GPS;
        }
    }
    ...
    {
        btnFilter.Enabled = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.TransferDataGPS_View);
        btnTransfer.Enabled = FunctionManager.CheckUserHasRight(CurrentUserRight, RightCode.TransferDataGPS_Transfer_Data);
    }
    ...
    public override void BindData()
    {
        ...
        */
        ...
        //DataTable dt = OperationProcessingManager.GetDataFromGPS(tuNgay, denNgay, idCongTy, idNhomTrangThietBi, idLoaiTrangThietBi,                                                                         txtFilterBienSo.Text.Trim());

        DataTable dt = OperationProcessingManager.GetDataFromGPS(tuNgay, denNgay);
        gridView.Columns.Clear();

        TemplateField field = new TemplateField();
        field.ShowHeader = true;
        field.HeaderTemplate = new GridViewTemplate(DataControlRowType.Header, "ID", "", "", "lblIDTrangThietBi", "center", 60);
        field.ItemTemplate = new GridViewTemplate(DataControlRowType.DataRow, "", "IDTrangThietBi", "Label", "", "left", 60);
    ...
    protected void gridView_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (row != null)
                {
                    HiddenField hfIDTrangThietBi = (HiddenField)e.Row.FindControl("hfIDTrangThietBi");
                    if (hfIDTrangThietBi != null && row["IDTrangThietBi"] != DBNull.Value)
                        hfIDTrangThietBi.Value = row["IDTrangThietBi"].ToString();

                    Label lblIDTrangThietBi = e.Row.FindControl("lblIDTrangThietBi") as Label;
                    if (lblIDTrangThietBi != null)
                        lblIDTrangThietBi.Text = row["IDTrangThietBi"].ToString();
                }
            }
        }
        catch (Exception exc)
    ...
    public void TransferDataFromGPS()
    {
        DateTime tuNgay = DateTime.MinValue;
        DateTime denNgay = DateTime.MinValue;
        ...
        OperationProcessingManager.TransferDataFromGPS(tuNgay, denNgay);
    }
    ...
    protected void btnTransfer_Click(object sender, EventArgs e)
    {
        try
        {
            TransferDataFromGPS();
            BindData();
            ShowMessage(this, "Transfer data successfully", MessageType.Message);
        }
        catch (Exception exc)
        {
            ShowMessage(this, exc.ToString(), MessageType.Error);
