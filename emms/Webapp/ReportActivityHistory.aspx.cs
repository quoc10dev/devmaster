...
public partial class ReportActivityHistory : BaseAdminGridPage
{
    public class GridViewTemplate : ITemplate
    {
        ...
        public void ctl_OnDataBinding(object sender, EventArgs e)
        {
            if (sender.GetType().Name == "Label")
            {
                ...
                //nếu số lượng < 0 thì tô màu đậm để cảnh báo 
                decimal number = 0;
                if (decimal.TryParse(((DataRowView)container.DataItem)[columnNameBinding].ToString(), out number))
                {
                    if (number < 0)
                    {
                        lb.ForeColor = System.Drawing.Color.Red;
                        lb.Font.Bold = true;
                        container.BackColor = System.Drawing.Color.Yellow;
                    }
                }
            }
    ...
    public override string FunctionPageCode
    {
        get
        {
            return FunctionCode.Report_Activity_History;
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "GridClick",
                "if(typeof(HighlightRow)!='undefined') HighlightRow(" + gridView.ClientID + ");", true);
        }
        catch (Exception exc)
        {
    ...
    public override void BindData()
    {
        ...
        //OperationProcessingManager.TinhTanSuatHoatDongThucTe(tuNgay, denNgay);
        DataTable dt = OperationProcessingManager.SearchActivityHistory(tuNgay, denNgay, idCongTy, idNhomTrangThietBi, idLoaiTrangThietBi, 
                                                                        txtFilterBienSo.Text.Trim());
        gridView.Columns.Clear();

        TemplateField field = new TemplateField();
        field.ShowHeader = true;
        field.HeaderTemplate = new GridViewTemplate(DataControlRowType.Header, "STT", "", "", "lblSTT", "center", 60);
        field.ItemTemplate = new GridViewTemplate(DataControlRowType.DataRow, "", "STT", "Label", "", "center", 60);
        ...
        field = new TemplateField();
        field.ShowHeader = true;
        field.HeaderTemplate = new GridViewTemplate(DataControlRowType.Header, "Loại xe", "", "", "lblLoaiXe", "center", 200);
        field.ItemTemplate = new GridViewTemplate(DataControlRowType.DataRow, "", "TenLoai", "Label", "", "left", 200);
        gridView.Columns.Add(field);

        field = new TemplateField();
        field.ShowHeader = true;
        field.HeaderTemplate = new GridViewTemplate(DataControlRowType.Header, "Biển số", "", "", "lblBienSo", "center", 100);
        field.ItemTemplate = new GridViewTemplate(DataControlRowType.DataRow, "", "BienSo", "Label", "", "left", 100);
        gridView.Columns.Add(field);

        List<string> colList = new List<string>();

        //Các cột ngày
        for (int i = 7; i <= dt.Columns.Count - 2; i++)
        {
            ...
            field.HeaderTemplate = new GridViewTemplate(DataControlRowType.Header, headerName, "", "", string.Format("{0}{1}", "lbl", columnName), "center", 80);
            field.ItemTemplate = new GridViewTemplate(DataControlRowType.DataRow, "", columnName, "Label", "", "center", 80);
            gridView.Columns.Add(field);
        }
        ...
        field = new TemplateField();
        field.ShowHeader = true;
        field.HeaderTemplate = new GridViewTemplate(DataControlRowType.Header, "Tổng cộng", "", "", "lblTotal", "center", 100);
        field.ItemTemplate = new GridViewTemplate(DataControlRowType.DataRow, "", "Total", "Label", "", "center", 100);
        gridView.Columns.Add(field);
        ...
        {
            gridView.Columns[1].Visible = false; //nhóm xe
            gridView.Columns[2].Visible = false; //loại xe
        }
        else
        {
            gridView.Columns[1].Visible = true; //nhóm xe
            gridView.Columns[2].Visible = true; //loại xe
        }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string url = string.Format("ReportActivityHistoryExport.aspx?FromDate={0}&ToDate={1}&IDCongTy={2}&IDNhomTrangThietBi={3}&IDLoaiTrangThietBi={4}&BienSo={5}", 
                string.Format("{0:yyyyMMdd}", tuNgay), 
                string.Format("{0:yyyyMMdd}", denNgay), 
